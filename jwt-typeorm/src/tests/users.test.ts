...
import { mockUser, mockUserInput, mockUserUpdateInput } from './fixtures';
...
describe('User', () => {
  describe('User / Service', () => {
    ...
    let getRepositorySpy: sinon.SinonSpy<[], UsersRepository>;

    beforeEach(() => {
      getRepositorySpy = sinon.spy(() => usersRepository);
    ...
    describe('User / Service / Read', () => {
      ...
      it('Read user by id', async () => {
        ...
        const res = await usersService.getById(userMock.id);
    ...
    describe('User / Service / Create', () => {
      it('Create user', async () => {
        ...
        const res = await usersService.create(userInputMock);
    ...
    describe('User / Service / Update', () => {
      it('Update user', async () => {
        ...
        const res = await usersService.update(userUpdateInputMock);

        expect(res).toEqual(updatedUserMock);
        expect(getById.calledOnce).toBe(true);
        expect(getById.getCall(0).args[0]).toEqual(userMock.id);
        expect(getRepositorySpy.calledOnce).toBe(true);
    ...
    describe('User / Service / Delete', () => {
      it('Delete user by id', async () => {
        ...
        const res = await usersService.delete(userMock.id);
